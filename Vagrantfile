...
Vagrant.configure("2") do |config|
  ...
  config.vm.define :vbox do |v|
    ...
    v.vm.provider :virtualbox do |virtualbox, override|
      ...
      virtualbox.customize ["storagectl", :id, "--name", "IDE_0", "--add", "ide"]
      virtualbox.customize ["storageattach", :id, "--storagectl", "IDE_0", "--port", "0", "--device", "0", "--type", "dvddrive", "--medium", "my-user-data.iso"]
